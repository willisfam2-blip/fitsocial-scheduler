...
async function save(job){await put(`${PREFIX}${job.id}.json`,JSON.stringify(job),{access:'public',contentType:'application/json',cacheControlMaxAge:60,addRandomSuffix:false,allowOverwrite:true,storeId:STORE_ID});}
...
export async function GET(request){
  ...
  const {blobs}=await list({prefix:PREFIX,limit:500,storeId:STORE_ID});const due=[];
  for(const blob of blobs){try{const r=await fetch(`${blob.url}?v=${Date.now()}`,{cache:'no-store'});const j=await r.json();if(j.status==='scheduled'&&new Date(j.scheduledAt)<=new Date())due.push(j)}catch{}}
  const results=[];
  for(const job of due.slice(0,5)){
    job.attempts=(job.attempts||0)+1;job.lastAttemptAt=new Date().toISOString();job.status='processing';await save(job);
  ...
  return Response.json({ok:true,storeIdConfigured:true,found:blobs.length,due:due.length,processed:results.length,results});
}
